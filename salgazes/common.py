...
def get_secrets(entry_name: str, secret_file_name: str = 'secret') -> Dict[str, str]:  # noqa: E501
    ...
    """
    with open(os.path.join(sgz.settings.root_dir, secret_file_name)) as f:
...
def get_configs(entry_name: str, config_file_name: str = 'config',
    ...
    try:
        with open(os.path.join(sgz.settings.root_dir, config_file_name)) as f:
            content = json.load(f)
    except FileNotFoundError:
        with open(os.path.join(sgz.settings.root_dir, config_default_file_name)) as f:  # noqa: E501
            content = json.load(f)
...
def save_to_p(file, data, desription_data='data'):
    ...
    """
    path = os.path.join(os.path.join(sgz.settings.root_dir, 'gazes'), file)
